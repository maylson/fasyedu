...
export default async function MuralPage() {
  ...
  return (
    <ModuleShell title="Mural de Avisos" description="Comunicações e recados institucionais">
      <div className="space-y-3">
        {data?.map((announcement) => (
          <article key={`${announcement.title}-${announcement.published_at}`} className="rounded-2xl border border-[var(--line)] bg-[var(--panel-soft)] p-4">
            <div className="flex items-center justify-between gap-3">
              <h3 className="font-semibold">{announcement.title}</h3>
              {announcement.is_pinned ? (
                <span className="rounded-full bg-[var(--accent)] px-2 py-1 text-xs font-medium text-[#113c66]">Fixado</span>
              ) : null}
            </div>
