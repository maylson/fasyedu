  ...
  return (
    <ModuleShell title="Dashboard" description="Resumo rápido da operação pedagógica">
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {cards.map((card) => (
          <article key={card.label} className="rounded-2xl border border-[var(--line)] bg-[var(--panel-soft)] p-4">
            <p className="text-sm text-[var(--muted)]">{card.label}</p>
            <p className="mt-2 text-3xl font-semibold text-[var(--brand-blue)]">{card.value}</p>
          </article>
        ))}
      </div>
    </ModuleShell>
